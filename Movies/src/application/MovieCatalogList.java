...
public class MovieCatalogList {
	...
	public ArrayList<Movie> sortByUserRating() {
        ArrayList<Movie> sortArray = new ArrayList<>(movies); 

        // Sort the new ArrayList using the SortHandle comparator
        Collections.sort(sortArray, new SortHandle());
...
class SortHandle implements Comparator<Movie> {
    ...
    public int compare(Movie m1, Movie m2) {
    	int num1 = Integer.parseInt(m1.userRating);
        int num2 = Integer.parseInt(m2.userRating);
        int numComparison = Integer.compare(num1, num2);
        if (numComparison != 0) {
            return -1*numComparison;
        }
        return 0;
    }
